...
AnimationInterface::WriteAnim ()
{
  ...
  parsedElement.version = 3.108; //доделать
...
/*void
AnimationInterface::parseGeneric (ParsedElement & parsedElement)
{
...
AnimationInterface::WriteAddNodeCounter (uint32_t counterId, std::string counterName, CounterType counterType){
  ...
  std::string Type = CounterTypeToString (counterType);// (не уверен что так)
  if (Type == "UINT32")
