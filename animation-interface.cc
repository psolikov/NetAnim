...
ParsedElement
AnimationInterface::WriteAnim ()
{
  ParsedElement parsedElement;
  parsedElement.type = XML_ANIM;
  parsedElement.version = NETANIM_VERSION;
...
AnimationInterface::WriteUpdateLink (uint32_t fromId, uint32_t toId, std::string linkDescription)
{
  ...
  parsedElement.linkDescription = linkDescription;
...
AnimationInterface::WriteLink (uint32_t fromId, uint32_t toLp, uint32_t toId)
{
  ...
  parsedElement.fromNodeDescription = lprop.fromNodeDescription;
  parsedElement.toNodeDescription = lprop.toNodeDescription;
  parsedElement.linkDescription = lprop.linkDescription;
  return parsedElement;
}
...
ParsedElement 
AnimationInterface::WriteP (uint64_t animUid, std::string pktType, uint32_t fId, double fbTx, double lbTx)
{
  if(pktType == "pr"){
    ParsedElement parsedElement;
                                 ...
                                 std::string metaInfo = "")
{
  ...
  if (pktType == "p"){
    ...
    parsedElement.meta_info = metaInfo;
    if (parsedElement.meta_info == "")
    {
      parsedElement.meta_info = "null";
    }
...
void 
AnimationInterface::WritingiteXmlPx (std::string pktType, uint32_t fId, double fbTx, double lbTx, 
...
AnimationInterface::WriteAddNodeCounter (uint32_t counterId, std::string counterName, CounterType counterType){
  ...
  parsedElement.nodeCounterName = counterName;
  QString counterType =  QString::fromStdString(CounterTypeToString (counterType));
...
AnimationInterface::WriteAddResource (uint32_t resourceId, std::string resourcePath)
{
  ...
  parsedElement.resourcePath = resourcePath;
  return parsedElement;
}
...
AnimationInterface::WriteUpdateNodeColor (uint32_t nodeId, uint8_t r, uint8_t g, uint8_t b)
{
  ...
  parsedElement.updateTime = Simulator::Now ().GetSeconds ().toString ().toDouble ();
  parsedElement.nodeId = nodeId;
  parsedElement.node_r = r;
  parsedElement.node_g = g;
  parsedElement.node_b = b;
  return parsedElement; 
}
...
AnimationInterface::WriteUpdateNodeDescription (uint32_t nodeId){
  ...
  if (m_nodeDescriptions.find (nodeId) != m_nodeDescriptions.end ())
    {
      parsedElement.nodeDescription = m_nodeDescriptions[nodeId];
    }
  return parsedElement; 
}
...
AnimationInterface::WriteUpdateBackground (std::string fileName, double x, double y, double scaleX, double scaleY, double opacity){
  ...
  parsedElement.fileName = fileName;
  parsedElement.x = x;
